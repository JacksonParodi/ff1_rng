/**
 * A simple implementation of the FF1 RNG algorithm
 */
class FF1Rng {
    constructor() {
        this.seed = 0;
        this.table = [
            ,
        ];

        /**
         * Set the RNG seed directly
         * @param {*} seed an integer between 0 and 255
         */
        this.set_seed = function (seed) {
            if (this.check_seed(seed)) {
                this.seed = seed % this.table.length;
            }
        }
        /**
         * Get the current RNG value and also advance the seed 
         * @returns the next integer in the RNG sequence
         */
        this.tick = function () {
            if (this.check_seed(this.seed)) {
                const next_int = this.table[this.seed];
                this.set_seed((this.seed + 1) % this.table.length);
                return next_int;
            }
            else {
                throw new Error('Seed is not valid');
            }
        }
        /**
         * Check if a seed is valid 
         * @param {*} seed an integer between 0 and 255
         * @returns true if the seed is valid, throw error otherwise
         */
        this.check_seed = function (seed) {
            if (Number.isInteger(seed) && seed >= 0 && seed < this.table.length) {
                return true;
            }
            throw new Error('Seed is not valid');
        }
    }
}
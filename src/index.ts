/**
 * A very simple reimplementation of Final Fantasy 1's random number generator
 */
export default class FF1Rng {
    ...
    /**
     * Sets the seed which will be used to index throw the table of 256 numbers
    ...
    /**
     * Gets the current random number, and advances the seed to the next number in the sequence
     * @returns The next number in the sequence, an integer between 0 and 255
     */
    public nextNumber(): number {
        const nextNumber = this.table[this.seed];
        this.seed = (this.seed + 1) % this.table.length;
        return nextNumber;
    }

    /**
     * Processes the seed input (number or string) to ensure it is a valid seed
     * @param seedInput - The seed input to process
